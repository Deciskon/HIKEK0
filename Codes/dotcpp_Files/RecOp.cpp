...
std::array<double,2> roof_CiPh2(int bins, double startt, double endd, int events, double DiluCon, double CSv, double Cintv, double Phi0v, double CLfrac, int loops)
{
    ...
    //decayfit<<CL*CLfrac<<"*exp(-x*"<<tauKS/tauKL<<") + CS*exp(-x*"<<tauKS/tauKS<<") + Cint*exp(-x*"<<tauKS/tauK<<")";
...
void RecOp()
{
	int noh = 100, events = (100000), loops = 50, i;
	double DiluCon = 1;//0.285714286;   //Dilucon = 2.0/7.0;  N(K0)/N(K0bar) = 1.8
	double CS = 0.43;//0.43
	double Cint = 0.4;//0.12;
